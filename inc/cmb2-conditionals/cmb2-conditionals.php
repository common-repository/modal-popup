...
add_action('plugins_loaded', 'cmb2_conditionals_load_actions_psmp');

function cmb2_conditionals_load_actions_psmp()
{
	if(!defined('CMB2_LOADED') || false === CMB2_LOADED) {
		return;
	}

	define('CMB2_CONDITIONALS_PRIORITY_PSMP', 99999);

	add_action('admin_init', 'cmb2_conditionals_hook_data_to_save_filtering_psmp', CMB2_CONDITIONALS_PRIORITY_PSMP);
	add_action('admin_footer', 'cmb2_conditionals_footer_psmp', CMB2_CONDITIONALS_PRIORITY_PSMP);
}

/**
 * Decides whether include the scripts or not.
 */
function cmb2_conditionals_footer_psmp()
{
	global $pagenow;
	...
	wp_enqueue_script('cmb2-conditionals_psmp', plugins_url('/cmb2-conditionals.js', __FILE__ ), array('jquery'), '1.0.2', true);
}

/**
 * Hooks the filtering of the data being saved.
 */
function cmb2_conditionals_hook_data_to_save_filtering_psmp()
{
	$cmb2_boxes = CMB2_Boxes::get_all();

	foreach($cmb2_boxes as $cmb_id => $cmb2_box) {
		add_action("cmb2_{$cmb2_box->object_type()}_process_fields_{$cmb_id}", 'cmb2_conditional_filter_data_to_save_psmp', CMB2_CONDITIONALS_PRIORITY_PSMP, 2);
	}
}
 ...
 */
function cmb2_conditional_filter_data_to_save_psmp(CMB2 $cmb2, $object_id)
{
